...
if [ -r "${tmp_dir}/package.tgz" ]; then
    ...
    grep -q 'scripts/run-packaged-first-use-test.sh' "${tmp_dir}/package-files.txt" && printf '%s\n' 'OK: target packaged first-use script present' || { printf '%s\n' 'MISSING: target packaged first-use script'; missing=$((missing + 1)); }
    ...
    grep -q 'scripts/run-packaged-httpd-example-test.sh' "${tmp_dir}/package-files.txt" && printf '%s\n' 'OK: target packaged HTTP example test present' || { printf '%s\n' 'MISSING: target packaged HTTP example test'; missing=$((missing + 1)); }
    ...
    grep -q 'run-packaged-first-use-test.sh' "${tmp_dir}/package/ui/index.html" && printf '%s\n' 'OK: DSM UI includes packaged first-use command' || { printf '%s\n' 'MISSING: DSM UI packaged first-use command'; missing=$((missing + 1)); }
    ...
    grep -q 'run-packaged-httpd-example-test.sh' "${tmp_dir}/package/ui/index.html" && printf '%s\n' 'OK: DSM UI includes packaged HTTP example command' || { printf '%s\n' 'MISSING: DSM UI packaged HTTP example command'; missing=$((missing + 1)); }
    ...
    grep -q 'PACKAGED FIRST USE TEST DRY RUN COMPLETE' "${tmp_dir}/package/scripts/run-packaged-first-use-test.sh" && printf '%s\n' 'OK: packaged first-use defaults to dry-run' || { printf '%s\n' 'MISSING: packaged first-use dry-run gate'; missing=$((missing + 1)); }
    grep -q 'PACKAGED FIRST USE TEST PASSED' "${tmp_dir}/package/scripts/run-packaged-first-use-test.sh" && printf '%s\n' 'OK: packaged first-use success marker present' || { printf '%s\n' 'MISSING: packaged first-use success marker'; missing=$((missing + 1)); }
    grep -q 'Packaged first-use test requires uid=0' "${tmp_dir}/package/scripts/run-packaged-first-use-test.sh" && printf '%s\n' 'OK: packaged first-use has root gate' || { printf '%s\n' 'MISSING: packaged first-use root gate'; missing=$((missing + 1)); }
    ...
    grep -q 'PACKAGED HTTP EXAMPLE TEST DRY RUN COMPLETE' "${tmp_dir}/package/scripts/run-packaged-httpd-example-test.sh" && printf '%s\n' 'OK: packaged HTTP example defaults to dry-run' || { printf '%s\n' 'MISSING: packaged HTTP example dry-run gate'; missing=$((missing + 1)); }
    grep -q 'PACKAGED HTTP EXAMPLE TEST PASSED' "${tmp_dir}/package/scripts/run-packaged-httpd-example-test.sh" && printf '%s\n' 'OK: packaged HTTP example success marker present' || { printf '%s\n' 'MISSING: packaged HTTP example success marker'; missing=$((missing + 1)); }
    grep -q 'hooks/httpd.example.sh' "${tmp_dir}/package-files.txt" && printf '%s\n' 'OK: target HTTP example hook present' || { printf '%s\n' 'MISSING: target HTTP example hook'; missing=$((missing + 1)); }
